...
gestures_vocab = syllables_to_gestures(vocabulary_list)
phoneme_to_index = {phoneme: idx for idx, phoneme in enumerate(unique(gestures_vocab))}
...
def prepare_data_for_videos_no_sliding_windows(base_names, phoneme_files, features_dir, labels_dir, 
    ...
    for base_name in base_names:
        if base_name in phoneme_files:
            ...
            syllables = syllabify_ipa(" ".join(phoneme_labels))
            gestures = syllables_to_gestures(syllables)
            syllable_labels = ["<SOS>"] + gestures + ["<EOS>"]
...
def train_model(model, train_loader, val_loader, optimizer, num_epochs, alpha, device):
    ...
    best_val_gesture_per = float('inf')  # Start with a very high value
    ...
    for epoch in range(num_epochs):
        ...
        decoded_val_sequences, true_val_sequences = decode_loader(
            model, val_loader, blank_token, index_to_phoneme, device, training=True
        )

        val_gestures_per = calculate_per_with_jiwer(decoded_val_sequences, true_val_sequences)
        ...
        wandb.log({"val_1-per (gestures)": 1 - val_gestures_per, "epoch": epoch + 1})
        # Save the model if it achieves the best validation PER
        if val_gestures_per < best_val_gesture_per:
            best_val_gesture_per = val_gestures_per
            best_epoch = epoch + 1
            # Save the model checkpoint
            model_save_path = f"/pasteur/appa/homes/bsow/ACSR/src/acsr/saved_models/model_gesture_decoding_last.pt"
            torch.save(model.state_dict(), model_save_path)
            print(f"New best model saved at epoch {best_epoch} with Val PER: {round(best_val_gesture_per, 3)}")
        sys.stdout.flush()
        
    print(f"Training complete. Best model saved at epoch {best_epoch} with Val PER: {round(best_val_gesture_per, 3)}")
...
if __name__ == "__main__":
    ...
    # Prepare DataLoaders
    train_loader = data_to_dataloader(train_data, batch_size=batch_size, shuffle=True)
    val_loader = data_to_dataloader(test_data, batch_size=batch_size, shuffle=False)
    ...
    print("Time to load one batch: ", round(end-start, 3))
    # Define the model
    ...
    #acoustic_model.load_state_dict(torch.load("/pasteur/appa/homes/bsow/ACSR/src/acsr/model_retrained_16_best_val_per.pt", map_location=device))
    #acoustic_model.load_state_dict(torch.load("/pasteur/appa/homes/bsow/ACSR/src/acsr/model_2_cuers_best_val_per.pt", map_location=device))
    acoustic_model.to(device)
    
    # Start training
    train_model(acoustic_model, train_loader, val_loader, num_epochs=epochs, alpha=alpha, device=device, optimizer=optimizer)
    torch.save(acoustic_model.state_dict(), "/pasteur/appa/homes/bsow/ACSR/output/saved_models/model_gesture_final.pth")
    ...
    decoded_val_sequences, true_val_sequences = decode_loader(acoustic_model, val_loader, blank_token, index_to_phoneme, device, training=False)
    ...
    print("Decoded validation gestures sequences:", decoded_val_sequences[-5:])
    print("True validation gestures sequences:", true_val_sequences[-5:])
    sys.stdout.flush()
    ...
    print("Validation PER (jiwer) gestures:", val_per, "1 - PER: ", 1 - val_per)
