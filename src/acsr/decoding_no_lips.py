...
def custom_collate_fn(batch):
    hand_shape, hand_pos, labels = zip(*batch)
    ...
    return hand_shape_padded, hand_pos_padded, labels_padded
...
class ThreeStreamFusionEncoder(nn.Module):
    def __init__(self, hand_shape_dim, hand_pos_dim, lips_dim, visual_lips_dim, hidden_dim=128, n_layers=2):
        ...
        self.fusion_gru = nn.GRU(hidden_dim * 4, hidden_dim * 2, n_layers, bidirectional=True, batch_first=True)
    ...
    def forward(self, hand_shape, hand_pos, lips, visual_lips):
        ...
        combined_features = torch.cat([hand_shape_out, hand_pos_out], dim=-1)
...
def joint_ctc_attention_loss(ctc_logits, attn_logits, target_seq, input_lengths, label_lengths, alpha, device):
    ...
    return total_loss, ctc_loss, None
...
def validate_model(model, val_loader, alpha, device):
    ...
    with torch.no_grad():
        for batch_X_hand_shape, batch_X_hand_pos, batch_y in val_loader:
            ...
            ctc_logits, attn_logits = model(
                batch_X_hand_shape, batch_X_hand_pos, None, None, target_seq=batch_y
            )
            ...
            total_attn_loss += 0 #attn_loss.item()
...
def train_model(model, train_loader, val_loader, optimizer, num_epochs, alpha, device):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_X_hand_shape, batch_X_hand_pos, batch_y in train_loader:
            ...
            epoch_attn_loss += 0 # attn_loss.item()
        ...
        if val_per < best_val_per:
            ...
            model_save_path = f"/pasteur/appa/homes/bsow/ACSR/src/acsr/saved_models/model_decoding_last_no_lips.pt"
...
def decode_loader(model, loader, blank, index_to_phoneme, device='cuda', training=False):
    ...
    with torch.no_grad():
        for batch_X_hand_shape, batch_X_hand_pos, batch_y in loader:
            batch_X_hand_shape = batch_X_hand_shape.to(device)
            batch_X_hand_pos = batch_X_hand_pos.to(device)
            #batch_X_lips = batch_X_lips.to(device)
            batch_y = batch_y.to(device)
            
            ctc_logits, _ = model(batch_X_hand_shape, batch_X_hand_pos, None, None)
...
if __name__ == "__main__":
    ...
    for batch_X_acoustic_hand_shape, batch_X_acoustic_hand_pos, batch_y in train_loader:
    ...
    acoustic_model = JointCTCAttentionModel(
        ...
        lips_dim=None,              # Number of lip keypoints
