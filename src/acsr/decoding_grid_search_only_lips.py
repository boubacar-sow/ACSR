    ...
    for base_name in base_names:
        if base_name in phoneme_files:
            ...
            train_videos_data[base_name] = {
                "X_acoustic_hand_shape": None,
                "X_acoustic_hand_pos": None,
...
def custom_collate_fn(batch):
    acoustic_lips, labels = zip(*batch)
    ...
    return acoustic_lips_padded, labels_padded
...
class ThreeStreamFusionEncoder(nn.Module):
    def __init__(self, hand_shape_dim, hand_pos_dim, lips_dim, visual_lips_dim, hidden_dim=128, n_layers=2):
        ...
        self.fusion_gru = nn.GRU(hidden_dim * 2, hidden_dim * 1, n_layers, bidirectional=True, batch_first=True)
    ...
    def forward(self, hand_shape, hand_pos, lips, visual_lips):
        ...
        combined_features = torch.cat([lips_out], dim=-1)
...
class JointCTCAttentionModel(nn.Module):
    def __init__(self, hand_shape_dim, hand_pos_dim, lips_dim, visual_lips_dim, output_dim, hidden_dim=128):
        ...
        self.attention_decoder = AttentionDecoder(encoder_dim=hidden_dim * 2, output_dim=output_dim)
        self.ctc_fc = nn.Linear(hidden_dim * 2, output_dim)
...
def validate_model(model, val_loader, alpha, device):
    ...
    with torch.no_grad():
        for batch_X_lips, batch_y in val_loader:
            ...
            ctc_logits, attn_logits = model(
                None, None, batch_X_lips, None, target_seq=batch_y
            )
...
def train_model(model, train_loader, val_loader, optimizer, num_epochs, alpha, device):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_X_lips, batch_y in train_loader:
        ...
        if val_per < best_val_per:
            ...
            model_save_path = f"/pasteur/appa/homes/bsow/ACSR/src/acsr/saved_models/gridsearch/lips_only_model_decoding_retrained{args.learning_rate}_{args.alpha}_{args.encoder_hidden_dim}_{args.n_layers}_{args.optimizer}.pt"
            optimizer_save_path = f"/pasteur/appa/homes/bsow/ACSR/src/acsr/saved_models/gridsearch/lips_only_optimizer_decoding_retrained2_{args.learning_rate}_{args.alpha}_{args.encoder_hidden_dim}_{args.n_layers}_{args.optimizer}.pt"
...
def decode_loader(model, loader, blank, index_to_phoneme, device='cuda', training=False):
    ...
    with torch.no_grad():
        for batch_X_lips, batch_y in loader:
            #batch_X_hand_shape = batch_X_hand_shape.to(device)
            #batch_X_hand_pos = batch_X_hand_pos.to(device)
            batch_X_lips = batch_X_lips.to(device)
            batch_y = batch_y.to(device)
            
            ctc_logits, _ = model(None, None, batch_X_lips, None)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--alpha", type=float, default=0.4, help="Weighting factor for CTC and attention losses")
    ...
    for batch_X_acoustic_lips, batch_y in train_loader:
    ...
    acoustic_model = JointCTCAttentionModel(
        hand_shape_dim=None, #batch_X_acoustic_hand_shape.shape[-1],  # Number of hand shape keypoints
        hand_pos_dim=None, #batch_X_acoustic_hand_pos.shape[-1],      # Number of hand position keypoints
    ...
    #acoustic_model.load_state_dict(torch.load("/pasteur/appa/homes/bsow/ACSR/src/acsr/saved_models/gridsearch/lips_only_model_decoding_retrained0.001_0.2_128_2_adam.pt", map_location=device))
    acoustic_model.to(device)
    
    # Start training
    train_model(acoustic_model, train_loader, val_loader, num_epochs=epochs, alpha=args.alpha, device=device, optimizer=optimizer)
    torch.save(acoustic_model.state_dict(), f"/pasteur/appa/homes/bsow/ACSR/src/acsr/saved_models/gridsearch/lips_only_model_decoding_last_retrained_{args.learning_rate}_{args.alpha}_{args.encoder_hidden_dim}_{args.n_layers}_{args.optimizer}.pt")
