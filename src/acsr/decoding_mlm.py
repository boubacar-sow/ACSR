...
from mlm import MaskedLMTransformer
...
def train_model(model, train_loader, val_loader, optimizer, num_epochs, alpha, device):
    ...
    for epoch in range(num_epochs):
        ...
        if val_per < best_val_per:
            ...
            model_save_path = f"/scratch2/bsow/Documents/ACSR/src/acsr/model_retrained_16_best_val_per.pt"
...
def compute_lm_score(seq, mlm_model, sos_idx, pad_idx, max_seq_len, device):
    if not seq:
        return 0.0
    lm_score = 0.0
    ...
    for i in range(1, len(seq)):
        ...
        lm_logits = mlm_model(input_tensor)  # shape: (1, vocab_size)
...
def greedy_decoder(output, blank, index_to_phoneme):
    ...
    for batch_idx in range(batch_size):
        ...
        for t in range(time_steps):
            ...
            if raw_info["token"] == "<UNK>" and raw_info["prob"] < 0.99: # if the model is not confident about the prediction of the blank token
...
def rescore_sequences_with_mlm(collapsed_decodes, raw_decodes, true_sequences, 
                               phoneme_to_index_acsr, phoneme_to_index_mlm, mlm_model, 
                               device, threshold=0.8, top_k=5, index_to_phoneme_mlm=None):
 
    import itertools

    pad_token = "<PAD>"
    mask_token = "<MASK>"
    pad_idx = phoneme_to_index_mlm[pad_token]
    mask_idx = phoneme_to_index_mlm[mask_token]
    max_seq_len = 15  # Adjust as needed
    ...
    for sample_idx, (collapsed_seq, raw_seq, true_seq) in enumerate(zip(collapsed_decodes, raw_decodes, true_sequences)):
        ...
        for token_info in collapsed_seq:
            if token_info['prob'] < threshold:
                # When the token is uncertain, use the mask token.
                alternatives.append([mask_token])
            else:
                alternatives.append([token_info['token']])
        
        # Generate all candidate sequences using the Cartesian product of alternatives.
        candidate_sequences = list(itertools.product(*alternatives))
        # Remove any pad tokens if present (optional)
        candidate_sequences = [[token for token in candidate if token != pad_token] for candidate in candidate_sequences]
        
        # Convert candidate sequences to MLM indices using phoneme_to_index_mlm.
        candidate_indices_list = []
        for candidate in candidate_sequences:
            indices = [phoneme_to_index_mlm.get(token, phoneme_to_index_mlm["<UNK>"]) for token in candidate]
            candidate_indices_list.append(indices)
        
        # Determine the maximum sequence length among the candidate sequences.
        max_L = max(len(seq) for seq in candidate_indices_list)
        # Pad each candidate sequence with pad tokens to reach max_L.
        padded_candidates = [seq + [pad_idx] * (max_L - len(seq)) for seq in candidate_indices_list]
        
        # Convert the list of padded candidate sequences to a tensor.
        candidate_tensor = torch.tensor(padded_candidates, device=device, dtype=torch.long)
        
        # Directly rescore the candidate sequences using the MLM.
        lm_scores, filled_sequences = direct_mlm_rescore(candidate_tensor, mlm_model, mask_idx, pad_idx, device)
        
        # Select the candidate sequence with the highest LM score.
        best_idx = torch.argmax(lm_scores).item()
        # IMPORTANT: Use the filled sequence (which has the masked tokens replaced)
        best_filled_indices = filled_sequences[best_idx].tolist()
        
        # If an index-to-token mapping is provided for the MLM, convert indices back to tokens.
        if index_to_phoneme_mlm is not None:
            best_sequence = [index_to_phoneme_mlm[idx] for idx in best_filled_indices if index_to_phoneme_mlm[idx] != pad_token]
        else:
            # Otherwise, return the indices.
            best_sequence = best_filled_indices
...
def direct_mlm_rescore(candidate_indices, mlm_model, mask_token_id, pad_idx, device):
 
    candidate_indices = candidate_indices.to(device)
    with torch.no_grad():
        mlm_logits = mlm_model(candidate_indices)  # [batch, seq_len, vocab_size]
        mlm_log_probs = F.log_softmax(mlm_logits, dim=-1)
    
    # Create mask for positions that are masked.
    mask_positions = (candidate_indices == mask_token_id)  # [batch, seq_len]
    # For each position, get the predicted token (argmax).
    predicted_tokens = mlm_log_probs.argmax(dim=-1)  # [batch, seq_len]
    # Gather log probabilities corresponding to the predicted token.
    predicted_log_probs = mlm_log_probs.gather(2, predicted_tokens.unsqueeze(2)).squeeze(2)  # [batch, seq_len]
    # Zero out positions that are not masked.
    masked_log_probs = predicted_log_probs * mask_positions.float()
    # Sum log probabilities at masked positions to compute a score per candidate.
    lm_scores = masked_log_probs.sum(dim=1)  # [batch]
    
    # Create filled sequences by replacing masked tokens with the predicted tokens.
    filled_sequences = candidate_indices.clone()
    filled_sequences[mask_positions] = predicted_tokens[mask_positions]
    
    return lm_scores, filled_sequences


def decode_loader_with_rescoring(model, mlm_model, loader, blank, index_to_phoneme, phoneme_to_index_acsr, phoneme_to_index_mlm, device='cuda'):
    model.eval()
    raw_decodes_all = []
    ...
    # Rescore sequences using MLM
    index_to_phoneme_mlm = {v: k for k, v in phoneme_to_index_mlm.items()}
    rescored_sequences = rescore_sequences_with_mlm(
        collapsed_decodes_all, raw_decodes_all, true_sequences,
        phoneme_to_index_acsr, phoneme_to_index_mlm, mlm_model, device, index_to_phoneme_mlm=index_to_phoneme_mlm
    )
...
if __name__ == "__main__":
    ...
    acoustic_model.load_state_dict(torch.load("/scratch2/bsow/Documents/ACSR/src/acsr/model_retrained_16_best_val_per.pt", map_location=device))
    ...
    #torch.save(acoustic_model.state_dict(), "/scratch2/bsow/Documents/ACSR/output/saved_models/model_epoch.pth")
    ...
    # Initialize model
    # Initialize and load the MLM model
    # insert MASK token at index 4
    phoneme_to_index_mlm = phoneme_to_index.copy()
    for index, key in enumerate(phoneme_to_index):
        if index >= 4:
            phoneme_to_index_mlm[key] += 1
    
    phoneme_to_index_mlm["<MASK>"] = 4
    index_to_phoneme_mlm = {index: phoneme for phoneme, index in phoneme_to_index_mlm
                        .items()}
    mlm_model = MaskedLMTransformer(
    ...
    mlm_model.load_state_dict(torch.load("best_syllable_mlm_model.pth", map_location=device))
    mlm_model.eval()
...
#    # Rescored decoding
    rescored_sequences, _ = decode_loader_with_rescoring(acoustic_model, mlm_model, val_loader, blank_token, index_to_phoneme, phoneme_to_index, phoneme_to_index_mlm, device)
#    # Calculate PER
